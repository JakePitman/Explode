...
export function AboutJake({ breakingState, setBreakingState }: AboutJakeProps) {
  ...
  const breakingCameraPositions = [
    new Vector3(
      originalCameraPosition.x - 3,
      originalCameraPosition.y + 1,
      originalCameraPosition.z - 1,
    ),
    new Vector3(
      originalCameraPosition.x + 3,
      originalCameraPosition.y + 1,
    ...
    new Vector3(
      ...
      originalCameraPosition.y - 3,
      originalCameraPosition.z - 1,
    ),
    new Vector3(
      originalCameraPosition.x + 4,
      originalCameraPosition.y - 2,
      originalCameraPosition.z - 1,
    ),
    new Vector3(
      originalCameraPosition.x - 4,
  ...
  const [nextCameraPositionIndex, setNextCameraPositionIndex] =
    useState<number>(0);
  ...
  useEffect(() => {
    ...
    if (breakingState === "original") {
      const indexes = breakingCameraPositions.map((_item, i) => i);
      indexes.splice(
        // Remove current index from potential indexes
        nextCameraPositionIndex,
        1,
      );
      ...
      setNextCameraPositionIndex(randomIndex);
    }
  }, [breakingState]);
  ...
  useFrame((state, delta) => {
    if (breakingState === "breaking") {
      ...
      state.camera.position.lerp(
        breakingCameraPositions[nextCameraPositionIndex],
        cameraLerpSpeed,
      );
    ...
    if (breakingState !== "breaking") {
      state.camera.position.lerp(originalCameraPosition, cameraLerpSpeed * 4);
    }
